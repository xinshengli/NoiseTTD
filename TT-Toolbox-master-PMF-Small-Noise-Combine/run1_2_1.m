function [RMSE, rank, time,res,density, uDensity, vDensity] = run1_2_1()
addpath(genpath('./'));
...
for j = 1:1
    for i =  1: 1
         ...
         [RMSE(i),~,rank(:,i),time(i),res(:,i), density(:,i), uDensity(:,i), vDensity(:,i)] = script(A,eps(i), randMisPos1,ssTensor,2);
          totalRMSE(i) = totalRMSE(i) + RMSE(i);
          totalTime(i) = totalTime(i) + time(i);
    end
end

save('totalRes1_2_1.mat','totalRMSE', 'totalTime');
